...
rm(list=ls())
setwd("/scratch/data01/BUSseq_cpp/Revision/GitHub_sub/NewHemat/BUSseq_nzf")
proj <- "hemat"

# load the raw count data
y_obs <- read.table(paste0("RawCountData/count_data_",proj,"_v2.txt"))

# load dimension information
dim <- unlist(read.table(paste0("RawCountData/dim_",proj,"_v2.txt")))
N <- dim[1]
G <- dim[2]
B <- dim[3]
nb <- dim[3 + 1:B]
...
zerorate_rep_nd <- read.table("ZeroRate_hemat_v2.txt")
...
zerorate_rep_or <- read.table("ZeroRate_hemat_v1.txt")
