...
# # w 
# file_w=paste0("MCMC_sampling_K",K,"/w_post.txt")
# con_w=file(file_w,open="r")
...
# gamma
file_gamma=paste0("MCMC_sampling_K",K,"/gamma_post.txt")
con_gamma=file(file_gamma,open="r")
...
for(i in 1:burnin){
  ...
  gamma.iter <- readLines(con_gamma, n = 1)
}
...
# for(i in 1:100){ 
  ...
  # w.temp <- as.numeric(unlist(strsplit(w.iter, split = " ")))
  ...
  gamma.temp <- matrix(as.numeric(unlist(strsplit(gamma.iter, split = " "))), 2, B)
  ...
  for(b in 1:B){
    for(i in 1:nb[b]){
      ...
      # w.temp[ind_cell] <- w.temp[ind_cell] + 1
      
      # mu_big = exp( alpha_g + beta_gk + nu_bg + delta_bi )
      mu.temp <- exp(alpha.temp + beta.temp[,w.temp[ind_cell]] + nu.temp[,b] + delta.temp[ind_cell])
      
      # x_big ~ NB(mu_big, phi_bg)
      x.temp[,ind_cell] <- rnbinom(G, size = phi.temp[,b], mu = mu.temp)
      
      # p_big = exp(gamma_b0 + gamma_b1 * x_big) / (1 + exp(gamma_b0 + gamma_b1 * x_big)) 
      temp <- gamma.temp[1,b] + gamma.temp[2,b] * x.temp[,ind_cell]
      prob.temp <- exp(temp) / (1 + exp(temp))
      
      # z_big ~ Bernoulli (p_big)
      z.temp[,ind_cell] <- rbinom(G,1,prob.temp)
      
      # y_big = x_big if z_big = 0
      y.temp[,ind_cell] <- x.temp[,ind_cell]
      
      # y_big = 0 if z_big
      y.temp[which(z.temp[,ind_cell] == 1), ind_cell] <- 0
  ...
  if(iter == num.output){
    ...
    write.table(zero_rate, file = paste0("PPC/ZeroRate_",proj,"_v",ver,".txt"), append = TRUE, row.names = FALSE, col.names = FALSE)
    iter <- 0
  }
  iter <- iter + 1
}
