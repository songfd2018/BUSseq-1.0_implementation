...
# Working directory
wkdir <- paste0("/scratch/data01/BUSseq_cpp/Revision/GitHub_sub/LUAD/Comparison/",method)
if(!dir.exists(wkdir)){
  dir.create(wkdir)
}
setwd(wkdir)
...
save.image(paste0(method, "_workspace.RData"))
...
###################################
scanorama_embedding <- read.table(paste0("scanorama_",proj,"_v",ver,"_integrated.txt"))
scanorama_corrected <- read.table(paste0("scanorama_",proj,"_v",ver,"_corrected.txt"))
...
##############################
w_scVI <- unlist(read.table(paste0("scVI_",proj,"_v",ver,"_clusters.txt")))
scVI_corrected <- read.table(paste0("scVI_",proj,"_v",ver,"_latent.txt"))
