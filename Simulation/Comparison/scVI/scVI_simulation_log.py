...
if not os.path.exists("./data"):
    os.mkdir("./data")
simulation_full=pd.read_csv("count_data_simulation_v4.txt",sep=" ",header=None)
simulation_dim=pd.read_csv("dim_simulation_v4.txt",sep=" ",header=None)

N = simulation_dim.at[0,0]
G = simulation_dim.at[1,0]
B = simulation_dim.at[2,0]

simulation_full.index = ["gene_"+str(i) for i in range(1,G+1)]
simulation_full.columns = ["sample_"+str(i) for i in range(1,N+1)]

# write count data into desired format

offset = 0
for b in range(B):
    simulation_full.iloc[:,offset:(offset+simulation_dim.at[3+b,0])].to_csv("./data/count_data_simulation_v4_batch"+str(b+1)+".csv",sep=",")
    offset += simulation_dim.at[3+b,0]

simulation_batch_1=CsvDataset("count_data_simulation_v4_batch1.csv", new_n_genes = G)
simulation_batch_2=CsvDataset("count_data_simulation_v4_batch2.csv", new_n_genes = G)
simulation_batch_3=CsvDataset("count_data_simulation_v4_batch3.csv", new_n_genes = G)
simulation_batch_4=CsvDataset("count_data_simulation_v4_batch4.csv", new_n_genes = G)
...
np.savetxt("scVI_simulation_v4_latent.txt", simulation_latent, fmt="%10.9f",delimiter="\t")
...
np.savetxt("scVI_simulation_v4_clusters.txt", simulation_clusters, fmt="%d",delimiter="\t")
...
np.savetxt("scVI_simulation_v4_de_genes.txt",simulation_de_genes, fmt="%s", delimiter="\t")