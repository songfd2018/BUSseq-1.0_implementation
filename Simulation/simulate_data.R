...
dim <- c(N,G,B,nb)
write.table(dim, file = "RawCountData/dim_simulation_v1.txt",row.names = FALSE, col.names = FALSE)

metadata <- data.frame( batch = paste0("Batch_",rep(1:B,nb)),
                        celltype = paste0("Type_",unlist(w)))
write.table(metadata, file = "RawCountData/metadata_simulation_v1.txt")
