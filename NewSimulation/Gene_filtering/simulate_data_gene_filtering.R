...
alpha.syn[1:100] <- 3.5
alpha.syn[G/5 + 1:100] <- 3
alpha.syn[G/5 * 2 + 1:50] <- 2.5
alpha.syn[G/5 * 3 + 1:50] <- 2.25
...
beta.syn[1:100,2] <- -2
beta.syn[101:200, 2] <- 2
beta.syn[G/5 + 1:50,2] <- -2
beta.syn[G/5 + 51:100,2] <- -1
beta.syn[G/5 + 101:150,2] <- 2 
beta.syn[G/5 * 2 + 1:50,2] <- -2
beta.syn[G/5 * 2 + 51:76,2] <- 2
beta.syn[G/5 * 3 + 1:50,2] <- -2
beta.syn[G/5 * 3 + 51:100,2] <- 2

#the cell-type effects of the third cell type
beta.syn[1:100,3] <- -2
beta.syn[101:200, 3] <- 2
beta.syn[G/5 + 1:51,3] <- -2
beta.syn[G/5 + 151:250,3] <- 2 
beta.syn[G/5 * 2 + 1:25,3] <- -2
beta.syn[G/5 * 2 + 151:200,3] <- 2 
beta.syn[G/5 * 3 + 1:50,3] <- -2
beta.syn[G/5 * 3 + 101:200,3] <- 2 

#the cell-type effects of the forth cell type
beta.syn[1:100,4] <- -2
beta.syn[201:250, 4] <- 2
beta.syn[G/5 + 1:100,4] <- -2
beta.syn[G/5 + 251:350,4] <- 2 
beta.syn[G/5 * 2 + 1:50,4] <- -2
beta.syn[G/5 * 2 + 201:300,4] <- 2  
beta.syn[G/5 * 3 + 1:50,4] <- -2
beta.syn[G/5 * 3 + 151:250,4] <- 2

#the cell-type effects of the fifth cell type
beta.syn[1:50,5] <- -1
beta.syn[51:100,5] <- -2
beta.syn[251:300, 5] <- 2 
beta.syn[G/5 + 1:100,5] <- -2
beta.syn[G/5 + 351:400,5] <- 2
beta.syn[G/5 * 2 + 1:50,5] <- -2
beta.syn[G/5 * 2 + 251:350,5] <- 2
beta.syn[G/5 * 3 + 1:50,5] <- -2
beta.syn[G/5 * 3 + 251:350,5] <- 2 
...
DE <- which(apply(abs(beta.syn) > 0, 1, sum) > 0)
DE_gene <- paste0("Gene_",DE)

phi.syn<-matrix(10,B,G)#mean 2 var 0.5
phi.syn[,(G*0.4 + 1):(G * 0.8)]<-5
phi.syn[,(G*0.8 + 1):G]<- 8
phi.syn[, DE] <- 2
...
hvg_batch <- list()
for(b in 1:B){
  rownames(y[[b]]) <- paste0("Gene_",1:G)
  colnames(y[[b]]) <- paste0("Cell_",1:nb[b])
  sc_exp <- SingleCellExperiment(assay = list(counts = y[[b]], logcounts = log1p(y[[b]])))
  clusters <- quickCluster(sc_exp, min.mean=0.1)
  sc_exp <- computeSumFactors(sc_exp, min.mean=0.1, clusters=clusters)
  sc_exp <- normalize(sc_exp)
  
  var.fit <- trendVar(sc_exp, method="loess", use.spikes=FALSE)
  var.out <- decomposeVar(sc_exp, var.fit)
  ...
  hvg_batch[[b]] <- rownames(hvg.out)
}

# Combined HVG
common.hvg <- rownames(y[[1]])
for(b in 1:B){
  common.hvg <- intersect(common.hvg, hvg_batch[[b]])
}

sum(DE_gene %in% common.hvg)
...
ind_hvg <- match(common.hvg,rownames(readcount))
read_hvg <- readcount[ind_hvg, ]
write.table(read_hvg, file = paste0("RawCountData/count_data_simulation_v",ver,".txt"),row.names = FALSE,col.names = FALSE)

dim <- c(N,length(common.hvg),B,nb)
...
log_mu_celltype <- alpha.syn[ind_hvg] + beta.syn[ind_hvg, ]
...
log_x <- NULL
for(b in 1:B){
  log_x <- cbind(log_x, log1p(x[[b]][ind_hvg,]))
}
...
log_y <- NULL
for(b in 1:B){
  log_y <- cbind(log_y, log1p(y[[b]][ind_hvg,]))
}
