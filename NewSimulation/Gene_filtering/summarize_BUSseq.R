...
y_obs <- read.table("RawCountData/count_data_simulation_v3.txt")

# Load dimension
dim <- read.table("RawCountData/dim_simulation_v3.txt")
dim <- unlist(dim)
N <- dim[1]
G <- dim[2]
B <- dim[3]
...
# Load metadata
metadata <- read.table("RawCountData/metadata_simulation_v3.txt")
...
for(k in k_sel){
  ...
  if(dir.exists(post_dir)){
    ...
    message(paste("Finish loading the results of ",proj,"_K",k,"!/n",sep=""))
  }else{
    message(paste("The results of ",proj,"_K",k," doesn't exist!/n",sep=""))
  }
}

jpeg("Image/BIC_values_v3.jpg",width = 540, height = 810)
...
break_mu <- seq(-1, 4, length.out = 12)
break_nu <- seq(-4, 4, length.out = 12)
break_logcount <- seq(0,6,length.out = 12)
...
jpeg("Image/heatmap_simulation_est_log_mean_expression_levels_v3.jpg",width = 1080, height = 1440)
...
jpeg("Image/heatmap_simulation_est_batch_effects_v3.jpg",width = 1080, height = 1440)
...
jpeg("Image/heatmap_simulation_log_imputed_count_v3.jpg",width = 1080, height = 1440)
...
jpeg("Image/heatmap_simulation_log_corrected_count_v3.jpg",width = 1080, height = 1440)
...
jpeg("./Image/scatter_simulation_cell_size_factors_v3.jpg",width = 540, height = 810)
...
BUSseq_by_celltype<- "Image/tsne_simulation_BUSseq_by_celltype_v3.jpg"
...
BUSseq_by_batch <- "Image/tsne_simulation_BUSseq_by_batch_v3.jpg"
dat_frame <- data.frame(Var1 = tsne_BUSseq_dist$Y[,1], 
                        Var2 = tsne_BUSseq_dist$Y[,2], 
                        col = batch_factor)
...
BUSseq_PCA<- "Image/pca_simulation_BUSseq_v3.jpg"
