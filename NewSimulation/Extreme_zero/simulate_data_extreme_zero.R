...
gamma.syn[4,]<-c(-0.75,-0.5)

#the log-scale baseline expression levels
alpha.syn<-rep(NA,G)
alpha.syn[1:(G/5)]<-rep(1,G/5)
alpha.syn[(G/5+1):(G/5*2)]<-rep(0.5,G/5)
alpha.syn[(G/5*2+1):(G/5*3)]<-rep(0,G/5)
alpha.syn[(G/5*3+1):(G/5*4)]<-rep(-0.25,G/5)
alpha.syn[(G/5*4+1):G]<-rep(-0.5,G/5)

alpha.syn[1:50] <- 3
alpha.syn[G/5 + 1:50] <- 2.5
alpha.syn[G/5 * 2 + 1:25] <- 2
alpha.syn[G/5 * 3 + 1:25] <- 1.75
...
beta.syn[G/5 + 26:50,2] <- -1
beta.syn[G/5 + 51:75,2] <- 2 
beta.syn[G/5 * 2 + 1:25,2] <- -2
beta.syn[G/5 * 2 + 26:75,2] <- 2
...
beta.syn[G/5 + 76:125,3] <- 2 
beta.syn[G/5 * 2 + 1:25,3] <- -2
beta.syn[G/5 * 2 + 76:100,3] <- 2 
beta.syn[G/5 * 3 + 1:25,3] <- -2
beta.syn[G/5 * 3 + 51:100,3] <- 2 

#the cell-type effects of the forth cell type
beta.syn[1:50,4] <- -2
beta.syn[101:125, 4] <- 2
beta.syn[G/5 + 1:50,4] <- -2
beta.syn[G/5 + 126:175,4] <- 2 
beta.syn[G/5 * 2 + 1:25,4] <- -2
beta.syn[G/5 * 2 + 101:150,4] <- 2  
beta.syn[G/5 * 3 + 1:25,4] <- -2
beta.syn[G/5 * 3 + 76:125,4] <- 2

#the cell-type effects of the fifth cell type
beta.syn[1:25,5] <- -1
beta.syn[26:50,5] <- -2
beta.syn[126:175, 5] <- 2 
beta.syn[G/5 + 1:50,5] <- -2
beta.syn[G/5 + 176:200,5] <- 2
beta.syn[G/5 * 2 + 1:25,5] <- -2
beta.syn[G/5 * 2 + 126:175,5] <- 2
beta.syn[G/5 * 3 + 1:25,5] <- -2
beta.syn[G/5 * 3 + 126:175,5] <- 2 
...
break_mu <- seq(-1.5, 3.5, length.out = 12)
break_nu <- seq(-5, 5, length.out = 12)
break_logcount <- seq(0,7,length.out = 12)
...
jpeg("Image/heatmap_simulation_col_key_mu_v33.jpg",width = 360, height = 270, quality =100)
...
jpeg("Image/heatmap_simulation_col_key_nu_v33.jpg",width = 360, height = 270, quality =100)
...
jpeg("Image/heatmap_simulation_col_key_read_count_v33.jpg",width = 360, height = 270, quality =100)
...
jpeg("Image/heatmap_simulation_log_mean_expression_levels_v33.jpg",width = 1080, height = 1440, quality =100)
...
jpeg("Image/heatmap_simulation_batch_effects_v33.jpg",width = 800, height = 1000, quality =100)
heatmap.3(nu.syn,
          dendrogram = "none",#with cluster tree
          Rowv = FALSE, Colv = FALSE,
          labRow = FALSE, labCol = FALSE,
...
jpeg("Image/heatmap_simulation_log_underlying_true_count_v33.jpg",width = 1080, height = 1440, quality =100)
heatmap.3(log_x,
          dendrogram = "none",#with cluster tree
          Rowv = FALSE, Colv = FALSE,
          labRow = FALSE, labCol = FALSE,
...
jpeg("Image/heatmap_simulation_log_observed_count_v33.jpg",width = 1080, height = 1440, quality =100)
heatmap.3(log_y,
          dendrogram = "none",#with cluster tree
          Rowv = FALSE, Colv = FALSE,
          labRow = FALSE, labCol = FALSE,
